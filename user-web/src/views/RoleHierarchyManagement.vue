<template>
  <v-row align="center">
    ...
    <FullDialog v-bind="addModalProps" v-on="{save:add, close: changeAddModal}">
      <template slot="body">
        <v-subheader inset>Role Hierarchy Information</v-subheader>
        <v-divider inset/>
        <v-row>
          <v-col cols="4">
            <v-subheader>Role ID</v-subheader>
          </v-col>
          <v-col cols="8">
            <v-text-field
                label="Role ID"
                v-model="addBody.roleId"
            />
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="4">
            <v-subheader>Parent Role ID</v-subheader>
          </v-col>
          <v-col cols="8">
            <v-text-field
                label="Parent Role ID"
                v-model="addBody.parentRoleId"
            />
          </v-col>
        </v-row>
      </template>
...
import {ADD_ROLE_HIERARCHY, GET_ROLE_HIERARCHIES, REMOVE_ROLE_HIERARCHY} from "@/api/modules/authorization-api";
...
export default {
  ...
  methods: {
    ...
    getGridData(pageNumber) {
      GET_ROLE_HIERARCHIES(this.gridProps.itemsPerPage, pageNumber, this.gridProps.searchType, this.gridProps.searchContent)
          .then(({data}) => {
            if (data.roleHierarchyList.content.length > 0) {
              this.gridProps.headers = makeDefaultGridHeaders(data.roleHierarchyList.content);
              const defaultGridItems = makeDefaultGridItems(data.roleHierarchyList.content);
              this.gridProps.items = defaultGridItems.items;
              this.gridProps.specialItems = defaultGridItems.specialItems;
              this.gridProps.pageNumber = data.roleHierarchyList.number + 1;
              this.gridProps.totalElements = data.roleHierarchyList.totalElements;
              this.gridProps.totalPages = data.roleHierarchyList.totalPages;
            }
          })
          .catch(this.CHANGE_FAIL_DIALOG);
    },
    add() {
      ADD_ROLE_HIERARCHY(this.addBody)
          .then(() => {
    ...
    remove() {
      ...
      REMOVE_ROLE_HIERARCHY(this.removeBody.item.id, getUserId())
          .then(() => {
            this.CHANGE_SUCCESS_DIALOG();
            this.getGridData(0);
          })
