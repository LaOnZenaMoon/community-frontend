  ...
  <v-row align="center">
    ...
    <FullDialog v-bind="addModalProps" v-on="{save:add, close: changeAddModal}">
      <template slot="body">
        ...
        <v-row>
          <v-col cols="4">
            <v-subheader>Resource ID</v-subheader>
          </v-col>
          <v-col cols="8">
            <v-text-field
                label="Resource ID"
                v-model="addBody.resourceId"
            />
          </v-col>
        </v-row>
      </template>
...
import {ADD_ROLE_RESOURCE, GET_ROLE_RESOURCES, REMOVE_ROLE_RESOURCE} from "@/api/modules/authorization-api";
...
export default {
  ...
  methods: {
    ...
    getGridData(pageNumber) {
      GET_ROLE_RESOURCES(this.gridProps.itemsPerPage, pageNumber, this.gridProps.searchType, this.gridProps.searchContent)
          .then(({data}) => {
            if (data.roleResourceList.content.length > 0) {
              this.gridProps.headers = makeDefaultGridHeaders(data.roleResourceList.content);
              const defaultGridItems = makeDefaultGridItems(data.roleResourceList.content);
              this.gridProps.items = defaultGridItems.items;
              this.gridProps.specialItems = defaultGridItems.specialItems;
              this.gridProps.pageNumber = data.roleResourceList.number + 1;
              this.gridProps.totalElements = data.roleResourceList.totalElements;
              this.gridProps.totalPages = data.roleResourceList.totalPages;
            }
          })
          .catch(this.CHANGE_FAIL_DIALOG);
    },
    add() {
      ADD_ROLE_RESOURCE(this.addBody)
          .then(() => {
    ...
    remove() {
      ...
      REMOVE_ROLE_RESOURCE(this.removeBody.item.id, getUserId())
          .then(() => {
            this.CHANGE_SUCCESS_DIALOG();
            this.getGridData(0);
          })
