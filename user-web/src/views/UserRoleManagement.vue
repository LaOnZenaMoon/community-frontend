<template>
  <v-row align="center">
    ...
    <FullDialog v-bind="addModalProps" v-on="{save:add, close: changeAddModal}">
      <template slot="body">
        <v-subheader inset>User Role Information</v-subheader>
        <v-divider inset/>
        <v-row>
          <v-col cols="4">
...
import {ADD_USER_ROLE, GET_USER_ROLES, REMOVE_USER_ROLE} from "@/api/modules/authorization-api";
...
export default {
  ...
  methods: {
    ...
    getGridData(pageNumber) {
      GET_USER_ROLES(this.gridProps.itemsPerPage, pageNumber, this.gridProps.searchType, this.gridProps.searchContent)
          .then(({data}) => {
            if (data.userRoleList.content.length > 0) {
              this.gridProps.headers = makeDefaultGridHeaders(data.userRoleList.content);
              const defaultGridItems = makeDefaultGridItems(data.userRoleList.content);
              this.gridProps.items = defaultGridItems.items;
              this.gridProps.specialItems = defaultGridItems.specialItems;
              this.gridProps.pageNumber = data.userRoleList.number + 1;
              this.gridProps.totalElements = data.userRoleList.totalElements;
              this.gridProps.totalPages = data.userRoleList.totalPages;
            }
          })
          .catch(this.CHANGE_FAIL_DIALOG);
    },
    add() {
      ADD_USER_ROLE(this.addBody)
          .then(() => {
    ...
    remove() {
      ...
      REMOVE_USER_ROLE(this.removeBody.item.id, getUserId())
          .then(() => {
            this.CHANGE_SUCCESS_DIALOG();
            this.getGridData(0);
          })
